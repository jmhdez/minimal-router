...
function findRouteParams(routes, path) {
	for (var i = 0; i < routes.length; i++) {
		var params = getMatchedParams(routes[i], path);
		if (params) {
...
var Router = function () {
	...
	_createClass(Router, [{
		...
		value: function getCurrentRoute(url) {
			var rp = findRouteParams(this.routes, parseUrl(url).path);
			return rp && rp.route;
		}
	}, {
		key: 'formatUrl',
		value: function formatUrl(routeName) {
			...
			var path = route.path.replace(parametersPattern, function (match) {
